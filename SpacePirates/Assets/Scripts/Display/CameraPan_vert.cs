...
public class CameraPan_vert : MonoBehaviour
{
	...
	private void MoveCamera(float moveAmt)
	{
		...
		if(moveAmt < 0.0f)
		{
			if(camTrans.localPosition.z > v2_bound_LowerUpper.x)
			{
				...
				if(camTrans.localPosition.z < v2_bound_LowerUpper.x)
				{
					...
					while(camTrans.localPosition.z > v2_bound_LowerUpper.x)
					{
		...
		if(moveAmt > 0.0f)
		{
			if(camTrans.localPosition.z < v2_bound_LowerUpper.y)
			{
				camTrans.Translate(v3_direction * moveAmt, Space.Self);
				...
				{
					camTrans.localPosition = tempCameraPos;
					while(camTrans.localPosition.z < v2_bound_LowerUpper.y)
					{
						camTrans.Translate(v3_direction * 0.5f, Space.Self);
					}
				}
